...
export default function EquipmentLabCanvas({
  ...
  useEffect(() => {
    ...
    const rimLight = new THREE.DirectionalLight(0xFF5E14, 3.0);
    rimLight.position.set(-5, -2, -4);
    scene.add(rimLight);

    const topSoft = new THREE.DirectionalLight(0xFF9E66, 1.4);
    ...
    const ringMat = new THREE.MeshBasicMaterial({ color: 0xFF5E14 });
  ...
  if (!hasWebGL) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center bg-[#0E1017] rounded-none border border-white/10 font-mono">
        <p className="text-sm font-semibold text-[#00FF66]">apparatus_3d_lab.obj</p>
        <p className="text-xs text-[#64748B] mt-1">[WEBGL_OFFLINE]</p>
      </div>
    );
  }

  return (
    <div className="relative w-full h-[400px] md:h-[480px] lg:h-[520px] rounded-none overflow-hidden bg-[#090A0F] border border-white/10">
      {/* 3D Canvas element */}
      <div
        ref={containerRef}
